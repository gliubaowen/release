...
{
echo `cat ${baseDir}/running/$1/main.pid`
...
{
echo "    |--->后台启动进程"
nohup java -DLOG_HOME=${2} -Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.port=${portJmx} -Dcom.sun.management.jmxremote.authenticate=false -Dcom.sun.management.jmxremote.ssl=false  -Duser.timezone=GMT+08  -Xms2048m -Xmx2048m -Xss512K -XX:PermSize=512m -XX:MaxPermSize=1024m -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=/home/netty/jvmdump -jar ${baseDir}/running/$1/*.jar start ${portStart} ${portStop} >/dev/null  &echo $! >  ${baseDir}/running/$1/main.pid
sleep 5
echo "        |--->[√]启动完成"
echo ""
}
